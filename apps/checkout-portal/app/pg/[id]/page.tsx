...
export default async function SelectPaymentMethod({
  ...
  const pgPaymentLink = await db.aPIGeneratedPaymentLink.findUnique({
    where: {
      id,
    },
    include: {
      ...
      <div className="container max-w-4xl px-4 py-8 md:py-12">
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <MerchantInfo
              logo={pgPaymentLink.merchant?.logoUrl}
              name={pgPaymentLink.merchant.name || ""}
            />
          </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                You are paying for the order from merchant{" "}
                {pgPaymentLink.merchant.name}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex justify-between items-center py-2">
                ...
                <span className="text-xl font-bold">
                  ${pgPaymentLink.amount.toFixed(2)}
                </span>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {stableCoins.map((coin) => (
              <CoinCard
                ...
                icon={coin.logoUrl}
                paymentId={pgPaymentLink.id}
                initiatedFrom="PG_PAYMENT"
              />
            ))}
          </div>
        </div>
